...
# from transformers import GemmaModel  # replace models/gemma in transformers
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    segment_size: int = field(default=16)

    infini_layer_idx: int = field(default=14)
...
def token_to_id(samples: dict, tokenizer) -> dict:
    ...
    input_ids = [
        np.array(item[0:768], dtype=map_dtype) for item in outputs["input_ids"]
    ]
...
def train():
    ...
    config = GemmaConfig.from_pretrained(
        model_args.model_name_or_path, attn_implementation="eager"
    )
